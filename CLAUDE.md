...
- 현재 스위트: `tests/test_net_room_auto.gd`(멀티 방 왕복 — 릴레이+호스트+게스트 3프로세스) · `tests/test_combat_math_auto.gd`(전투 신뢰 경계 단위 — 사거리·쿨다운·구르기 i-frame·잔몹 타격) · `tests/test_game_state_auto.gd`(직업 리졸버 — data/jobs 스캔 allowlist + 조작 id 폴백) · `tests/test_health_component_auto.gd`(HP·부활 권한/표시 경로 격리). 실행법은 `projectb-verify` §1. 판정 = `TEST_OK` + exit 0 + `SCRIPT ERROR` 없음.
