...
- **챕터1 보스 = "몰락한 망령"(64px) 하나다** — `data/enemies/wraith_boss.tres` · 씬 `src/enemies/boss.tscn`(노드 `Boss`) · 스크립트 `src/enemies/boss.gd`(**어떤 `BossDef`든 돌리는 데이터 주도 공용 배우** — 새 보스 = `.tres` + 시트 한 장). ⚠ **거대 악어는 2026-07-26에 완전 제거했다**(아트·데이터·`boss_croc` 이름 일체) — 미배선 악어와 배선된 망령이 공존해 문서·코드가 계속 혼선을 만들었다. 옛 이력(`docs/CHANGELOG.md`·메모리)에 "거대 악어"가 나오면 그 시점 기준이다. 🔴 **알려진 결함 3건**(전부 2026-07-26 재확인, 미해소 — 상세·해소 선택지는 `projectb-art` 「보스」 절): ⑴ 콘 텔레그래프 텍스처가 **68.6°** 로 그려져 있는데 망령 판정은 **85.9°** → **좌우 각 8.6°가 무예고 피격**(`projectb-rules` §3, 사용자 결정 = 텍스처 86° 재작화) ⑵ 망령 시트에 `walk`가 없어 이동 시 idle로 미끄러진다 ⑶ **공격 애니가 예고 시간의 3분의 1에서 끝난다** — `swing` 0.42s / `slam`·`spray` 0.50s인데 `telegraph_s`는 1.2~1.5s라, 보스가 휘두름을 마치고 **idle 자세로 0.8~1초 서 있다가 타격**한다(§3 "애니 길이 ≈ telegraph_s" 미러 깨짐, 에러 없음).
- 🔴 **다음 착수 지점 = 아트 생산 (판 깔기는 끝났다).** 네 작업 모두 `assets/aseprite/`에 원본이 있어 **즉시 시작 가능**하다(2026-07-26 확인). 권장 순서 = ④ → ① → 콘 각 결정 → ② (사용자 미확정이라 물어보고 시작).
  ...
  - 🔴 **보스 결함 3건:** ⑴ 콘 각 — **사용자 결정 = 86° 텍스처 재작화**(2026-07-26, 착수 전) ⑵ 망령 `walk` 4프레임 추가(시트 1536×64 → 1792×64) ⑶ **공격 애니가 `telegraph_s`보다 0.8~1초 짧다** — 해소 = tres speed를 내리거나(데이터 3줄) 프레임을 12~15로 늘린다(아트). 상세는 `projectb-art` 「보스」 절.
