...
- **상태:** 기획 확정(GDD v1.7, 해커톤 마감 2026-08-10) · 아키텍처 규칙 확정(`projectb-rules` §1~§5) · **전투 코어·챕터1 골격·손맛/오디오 완료.** 진행 중 = **드랍→제작/강화 루프**(브랜치 `feat/loot-crafting`) — 슬라이스 0(core 토대: 스키마·SaveManager·인벤·장비 계약·드랍 메시지·데이터) 완료·헤드리스 4종 그린. 다음 = 슬라이스 1(드랍 엔티티·선착 픽업 네트워킹). 기준 해상도 640×360은 임시값(GDD §9 TBD).
