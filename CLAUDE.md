...
- 🔵 **챕터1 보스 = "몰락한 망령"(64px)이 현행 배선이다** — `stage_boss.tscn`의 `BossCroc` 노드가 `wraith_boss.tres`를 문다(씬 이름의 `croc`은 과거 이름). `croc_boss.tres`(거대 악어 72px, 31프레임)는 아트·데이터 완성 상태로 **미배선**이다. ⚠ 옛 문서·이력이 "거대 악어"로 적어 둔 것은 그 시점 기준이다. 🔴 **알려진 결함 2건**: ⑴ 콘 텔레그래프 텍스처가 악어 각(68.6°) 기준인데 망령 판정은 85.9° → **좌우 각 8.6°가 무예고 피격**(`projectb-rules` §3) ⑵ 망령 시트에 `walk`가 없어 이동 시 idle로 미끄러진다.
