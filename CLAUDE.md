...
- 🔴 **다음 착수 지점 = 아트 생산 (판 깔기는 끝났다).** 네 작업 모두 `assets/aseprite/`에 원본이 있어 **즉시 시작 가능**하다(2026-07-26 확인). ✅ **보스 결함 3건은 2026-07-26에 해소 완료**(위 보스 줄) — 남은 것은 아래 ①~④다. 권장 순서 = ④ → ① → ② (사용자 미확정이라 물어보고 시작).
  ...
  - ✅ **보스 결함 3건 = 해소 완료 (2026-07-26).** 망령 시트는 이제 **2368×64 / 37프레임 / 애니 6종**(idle 4 · walk 4 · swing·slam·spray 8 · death 5)이고 `telegraph_cone.png`는 **80×110 정원 부채꼴**이다. ⚠ **여기서 얻은 교훈 두 개가 다음 아트 작업에 그대로 적용된다:** ⑴ **콘 텍스처 계약은 조항이 넷**이다(정원 부채꼴·반지름=`tex_w`·전체각=2×`half_angle`·테두리 내측) — 문서가 "각도"만 적어 둬서 리드가 삼각형으로 오독했고, art가 실측으로 반박해 막았다 ⑵ **기하 검증은 "어느 알파 계층·어느 x 범위"를 밝혀라** — apex 근처 테두리 픽셀을 잡으면 정확한 텍스처가 2° 위반으로 보인다(리드가 그래서 불필요한 재작화를 지시했다가 철회).
