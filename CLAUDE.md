...
- **남은 것 (아트 = 사용자가 고른 순서):** ① **idle 애니 2→4프레임**(3직업 6장 — 현재 2프레임 2.9fps라 뻣뻣하다. 32px 재작화보다 싸고 체감이 크다) ② **팔레트 정본화**(= `boss_arena.png`·`swamp_ground.png` 2장 재작화 + 그룹별 실측색 정리) ③ **placeholder 교체**(드랍 아이콘 등) ④ **죽은 세대 PNG 29장 정리**(규격 확정됐으니 안전 — 목록은 `projectb-art`). 그 외: 보스 결함 2건(콘 각·walk 애니) · 손맛 수치 실기 튜닝(`docs/TUNING.md`) · **실기 확인 몫**(웹 2클라: 드랍/픽업·제작 루프·모닥불·전멸→마을 롤백·IndexedDB 저장·SFX·I키 인벤 클릭 도달 · **P2P "직결" 표시 + 배포본에서 4분 이상 유지** · 4방향 전환·무기 그립·확대 배율·이동 체감).
