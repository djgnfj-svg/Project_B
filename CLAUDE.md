...
- **챕터1 보스 = "몰락한 망령"(64px) 하나다** — `data/enemies/wraith_boss.tres` · 씬 `src/enemies/boss.tscn`(노드 `Boss`) · 스크립트 `src/enemies/boss.gd`(**어떤 `BossDef`든 돌리는 데이터 주도 공용 배우** — 새 보스 = `.tres` + 시트 한 장). ⚠ **거대 악어는 2026-07-26에 완전 제거했다**(아트·데이터·`boss_croc` 이름 일체) — 미배선 악어와 배선된 망령이 공존해 문서·코드가 계속 혼선을 만들었다. 옛 이력(`docs/CHANGELOG.md`·메모리)에 "거대 악어"가 나오면 그 시점 기준이다. 🔴 **알려진 결함 2건**: ⑴ 콘 텔레그래프 텍스처가 **68.6°** 로 그려져 있는데 망령 판정은 **85.9°** → **좌우 각 8.6°가 무예고 피격**(`projectb-rules` §3) ⑵ 망령 시트에 `walk`가 없어 이동 시 idle로 미끄러진다.
- 🔴 **다음 착수 지점 = 아트 생산 (판 깔기는 끝났다).** 네 작업 모두 `assets/aseprite/`에 원본이 있어 **즉시 시작 가능**하다(2026-07-26 확인). 권장 순서 = ④ → ① → 콘 각 결정 → ② (사용자 미확정이라 물어보고 시작).
  ...
  - ③ ⚠ **드랍 아이콘 "placeholder"는 재확인 대상** — 실측은 5~8색·채움 28~47%로 **정상 범위**다(옛 서술이 낡았을 가능성). `export_frame scale 8`로 눈으로 보고 판단한다. 참조 0인 `ui/icon_weapon.png`만 진짜 미완성으로 보인다.
  - ④ **죽은 세대 PNG 정리** — 규격 확정 전에는 16px/32px 실물 비교에 필요해서 남겼다. 이제 안전(목록·판별 명령은 `projectb-art`). ⚠ `fx/coop_*` 9장은 **미착수**라 지우지 않는다.
  - 🔴 **보스 결함 2건 (사용자 결정 필요):** 콘 각(데이터 한 줄 vs 86° 텍스처 신규) · 망령 `walk` 4프레임 추가(시트 1536×64 → 1792×64).
...
- **에셋(도트) = `projectb-art`** — ⚠ 규격 정본 미정, 합의 전 신규 스프라이트 착수 금지.
