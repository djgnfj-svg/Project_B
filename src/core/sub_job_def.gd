...
# 계열(series_id = JobDef.id) 안의 성장 갈래 — 무기·모션을 바꾸지 않는다 (GDD §5).
# ⚠ **2026-08-02에 스프라이트 조항이 좁혀졌다** — 아래 `body_frames`가 **같은 시트의 색 치환본**을
#   물릴 수 있다(사용자 확정 = "옷 색만"). 실루엣·프레임·애니 이름은 그대로이므로 "모션을 바꾸지
#   않는다"는 유지된다. 🔒 **GDD §5는 아직 옛 문구다**(승인제라 미반영).
#
...
# 몸 스프라이트 — null = 직업 기본 시트(`JobDef.frames`) = 도입 전과 항등. (2026-08-02)
# 사용자 확정 = **"옷 색만 갈아입힌다"** — 같은 시트를 색만 치환한 것이라 픽셀 배치·프레임 폭·
#   애니 이름 집합이 원본과 **동일**하다. 그래서 이 교체는 판정·애니 배선을 한 칸도 안 움직인다.
# 🔴 **위 「축 경계」의 예외가 아니다 — 여전히 표시 전용이다.** 실루엣이 같으므로 히트박스·
#   `body_radius`·모션 어디에도 닿지 않는다. 🔴 **실루엣이 다른 시트를 여기 물리지 마라**:
#   판정은 그대로인데 그림만 커지면 "몸통에 맞췄는데 안 맞는다"가 되고 **화면에 이유가 안 드러난다**
#   (`EnemyDef.sprite_scale` ↔ `body_radius` 함정과 같은 부류, CLAUDE.md).
# 🔴 **애니 이름 집합이 직업 기본 시트와 같아야 한다.** 다르면 `_play_dir_anim`이 **에러 없이**
#   2방향으로 폴백하거나 클립을 못 찾는다(rules §5 4방향 함정). 전수 트립와이어가 지킨다.
# 🔴 네트워크로 오가는 것은 여전히 **id뿐**이다(G_STATS "ms") — 시트는 각 클라가 자기 data/subjobs에서
