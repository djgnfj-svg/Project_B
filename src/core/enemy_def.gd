...
# 스프라이트 배율 — 시트를 다시 그리지 않고 덩치만 키운다 (2026-08-01, 소를 크게 해달라는 요청).
# 🔴🔴 **이 값을 바꾸면 `body_radius`도 같이 바꿔라.** 판정은 여전히 `body_radius`가 정하므로,
#   그림만 키우면 **"몸통에 맞췄는데 안 맞는다"** 가 된다(§3 "맞는 곳 = 보이는 곳"의 위반이고,
#   화면에 이유가 전혀 안 드러난다). 그림자·텔레그래프·화살 총구·길찾기 반경은 전부 `body_radius`에서
#   유도되므로 그쪽만 맞추면 **나머지는 자동으로 따라온다**.
# ⚠ 이 미러는 새로 생긴 위험이 아니다 — 위 `body_radius` 주석이 이미 "스프라이트 크기와 정합 유지"를
#   사람에게 요구하고 있었다(시트를 다시 그릴 때마다 손으로 맞추던 자리). 배율은 그 축을 데이터로
#   옮긴 것뿐이다.
# ⚠ 픽셀아트라 **정수배가 가장 깨끗**하다. 비정수배는 픽셀 크기가 불균일해지지만(1.4배면 일부 픽셀만
#   2px) 32px 캐릭터 옆에서 덩치 차이가 읽히는 쪽이 더 중요하다고 판단한 값이다 — 실기에서 지저분하면
#   1.0(항등)이나 2.0으로 옮겨라.
