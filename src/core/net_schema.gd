...
const G_ATK := "atk"                  # {k, dx, dy, cb} 공격 연출 (방향 + 콤보 타수) — 판정 아님, 원격 표시용. cb는 궤적만 정한다(수신부 clamp)
...
# 🔴 cb = 평타 콤보 타수(궁수 "평·평·쭉", 2026-07-27). **G_ATK의 "cb"와 성격이 다르다** — 저쪽은 궤적만 정해
#   조작돼도 화면이 달라질 뿐이지만, 이 타수는 **사거리·데미지 배율을 고른다**. 그래서 호스트는 이 값을
