...
const G_SKILL := "skill"              # {k, dx, dy}   발신자=본인: 하위 직업 스킬 발동 선언 (2026-08-02). 🔴 **수치는 한 칸도 안 싣는다** — 배율·반경·사거리·쿨다운은 호스트가 그 피어의 공지 하위 직업(G_STATS "ms")으로 자기 data/subjobs → data/skills에서 리졸브한다(`peer_weapon_id`·`projectile_params`와 같은 철학, rules §3). 실으면 그게 곧 "내 스킬은 ×99"라고 주장하는 표면이다. dx/dy = 발동 방향(beam 축·spin 표시 회전) — G_HIT_REQ의 dx/dy와 같은 등급의 **클라 주장**이라 판정 원/띠의 크기는 못 넓힌다(방향만 고른다). 호스트는 ⑴ 생존 ⑵ 그 피어가 그 스킬을 실제로 메인에 꼈는가(계열 게이트 = `GameState.main_skill_of`) ⑶ 쿨다운(`is_skill_ready` — 자기 수신 시각 기준, `is_fire_rate_ok` 규약 미러)를 검증한 뒤 판정한다. 데미지 결과는 **새 경로를 만들지 않고 기존 G_ENEMY_HP로 흐른다**. 🔴 사건이라 유실 허용(fast) 채널 금지 — 한 통이 사라지면 쿨다운은 도는데 아무 일도 안 일어난다
