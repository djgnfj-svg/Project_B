...
#     ③ **칼끝 리본 + 칼 잔상**(현재, `player.SwingTrail` · `AfterImage.spawn_weapon`) — 칼끝 좌표와
#        칼 스프라이트를 **그대로** 쓴다. 어긋날 축이 존재하지 않아 정합이 구조로 보장된다(실측 1.03px).
#   ⚠ 그래서 `swing_fx_*` 배율 3종도 07-29에 삭제했다 — 리본은 칼을 따라갈 뿐이라 조절할 "형태"가 없다.
#     궤적 굵기·꼬리는 `player.tscn`의 `SwingTrail`(width·Gradient·Curve)이 정본이고, 무기별로
#     가르고 싶어지면 **그때 다시 데이터 자리를 만들어라** — 지금 없는 것은 필요가 없어서다.
