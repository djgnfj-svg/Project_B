...
# 무기가 **회전하는 중심**(캐릭터 원점 기준 px) — 대검을 어깨에 멘 것처럼 보이게 하는 자리다(사용자 요청 2026-07-28).
# ⚠ 기본값은 `player.gd`의 `WEAPON_PIVOT_DEFAULT`와 **같아야 한다** — 미착용(무장 해제) 때 그쪽 상수가 쓰이므로,
#   두 값이 갈라지면 "맨손일 때와 무기 낄 때 회전 중심이 다르다"가 에러 없이 생긴다.
# ⚠ 판정(부채꼴 apex)은 여기와 **무관하게 캐릭터 원점**이다 — 표시만 옮긴다. 그래서 이 값을 크게 주면
#   칼이 나가는 곳과 맞는 곳이 벌어진다. 몇 px 안에서 쓴다.
#   (궤적 리본·잔상은 이 피벗을 그대로 따라가므로 **칼과는 안 갈라진다** — 갈라지는 것은 판정과의 사이다.)
@export var weapon_pivot: Vector2 = Vector2(0, -4)
# 등(어깨)에 메는 무기인가 — 앞뒤 그리기 순서가 **손에 든 무기와 정반대**가 된다(사용자 확정 2026-07-28).
#   손에 든 무기: 아래 조준(정면) = 손이 앞 → 몸 앞 · 위 조준(뒤통수) = 몸 뒤
#   등에 멘 무기: 아래 조준(정면) = 검이 등 뒤 → **몸 뒤** · 위 조준(뒤통수) = **몸 앞**
# ⚠ 스윙 중에도 같은 규칙이다 — 휘두르는 동안만 앞으로 꺼내면 z가 한 번 튀어 깜빡이는 것처럼 보인다.
@export var weapon_on_back: bool = false
# 🔴 **칼날이 시작하는 지점 — 칼끝에서 뒤로 재는 길이(px).** 궤적 리본이 채우는 띠의 **radial 폭**이다
#   (칼날 폭 리본 2026-08-01): 리본은 `[칼끝 − 이 값, 칼끝]` 사이만 채우고 **칼끝 바깥으로는 한 픽셀도
#   안 나간다**(나가면 도달 거리를 오해하게 만든다 — 안쪽으로 넓어지는 것만 안전한 방향이다).
# 🔴 **`weapon_grip`·`weapon_hold_dist`·`weapon_pivot`과 같은 그룹이다 — 스프라이트 리깅 데이터.**
#   "어디를 쥐는가"를 데이터로 적고 있으므로 "어디까지가 날인가"도 같은 자리다. 무기를 다시 그리면
#   그립을 다시 잡는 그 순간에 이 값도 같이 잡는다(새 미러 계층이 생기는 것이 아니다).
# 🔴 **왜 텍스처 픽셀에서 유도하지 않았나** (2026-08-01 실측 판단): 열 두께로 「날」을 찾는 휴리스틱은
#   창 15 · 도끼 13 · 지팡이 7을 맞히지만 **대검에서 우연히 맞는다** — 대검은 날 두께 7 · 손잡이 5로
#   거의 같고 가장 두꺼운 칸이 날이 아니라 **코등이**다. 한 무기가 우연이면 다음 아트 교체에서
#   조용히 뒤집힌다. 폴백·상한·기본값은 `CombatMath.blade_length()`가 쥔다(전수 트립와이어 대상).
# ⚠ 0 = `CombatMath.DEFAULT_BLADE_LENGTH`(옛 칼끝 리본 굵기 ≈ 7px) = **도입 전 근사 항등**.
#   발사형(활·지팡이)은 휘두르지 않으므로 비워 둔다.
@export var blade_length: float = 0.0
...
# 마무리 타의 **판정** 반각(rad). 0 = `swing_arc` 그대로 = **판정 축에서는 도입 전과 항등**.
# ⚠ **표시는 항등이 아니다**(리뷰 B-2) — `melee_show_half_angle`은 `is_finish`면 이 값이 0이어도
#   `COMBO_FINISH_SHOW_MARGIN`을 붙인다. 실측: 창 = 판정 0.3 / 표시 0.45라 마무리 젖힘이 **+8.6°**
#   눈에 보인다(도끼는 표시가 어깨걸치기 `cap`에 이미 포화해 화면 차이 0). 방향은 안전하다(표시 > 판정).
...
# 마무리 타의 **판정 도달 거리(px, 절대값)** — 0 = 없음 = 도입 전과 완전 항등. (GDD v2.3 §6)
# 🔴 **절대값이지 배율이 아니다 — `combo_finish_arc`와 같은 이유이되 포화 지점이 다르다.**
#   각은 문턱이 `π ÷ 반각`이라 무기마다 달랐고, 사거리는 **천장이 무기마다 다르다**(기본 도달 × 2:
#   도끼 76 · 대검 84 · 창 84). 균일 배율 ×1.5를 쓰면 대검·도끼는 통과하는데 **창만 조용히 절삭**된다
#   (80 × 1.5 = 120 → 84로 잘려 실효 ×1.05). 그래서 무기가 픽셀을 직접 적는다.
# 🔴 **상한은 데이터가 아니라 `effective_attack_range`의 `minf`가 쥔다**(기본 도달 × `HIT_REACH_SLACK`).
#   그 clamp 하나가 「로컬 질의 ⊆ 호스트 수용」을 **구조로** 만든다 — 크게 적으면 조용히 잘릴 뿐
#   거부 띠는 생기지 않는다(전수 트립와이어가 절삭 자체를 잡는다).
# ⚠ 창은 참여하지 않는다(0) — 천장 84가 기본 80의 1.05배뿐이라 의미가 없고, 창의 마무리 보상은
#   `combo_dash`(몸이 나가는 돌진)다.
@export var combo_finish_range: float = 0.0
...
@export_group("무기 손맛")
# 🔴 **궤적의 형태를 정하는 데이터 자리는 없다 — 일부러 없앴다** (2026-07-29 최종).
#   세 세대를 거쳤고 앞의 둘은 전부 "칼과 빛이 따로 논다"로 끝났다:
#     ① 텍스처(`swing_texture`/`swing_tex_radius`, ~07-28) — 각·도달이 PNG 픽셀에 박혀 `swing_arc`를
#        튜닝할 때마다 갈라졌다(실측 worn +45° / iron +61°). 창(±17°)은 아예 성립 불가.
#     ② 부채꼴 셰이더(`swing_arc.gdshader`, 07-28~07-29) — 판정 데이터에서 직접 그려 각은 맞췄지만,
#        **그리는 것이 "칼이 지나간 영역"(부채꼴 띠)이고 칼은 그 테두리 한 줄**이라 형태가 애초에
#        달랐다. 각을 3°까지 맞춰도 포개지지 않는다.
#     ③ **칼끝 리본 + 칼 잔상**(07-29, `player.SwingTrail` · `AfterImage.spawn_weapon`) — 칼끝 좌표와
#        칼 스프라이트를 **그대로** 쓴다. 어긋날 축이 존재하지 않아 정합이 구조로 보장된다(실측 1.03px).
#     ④ **칼날 폭 리본**(현재, 2026-08-01) — ③의 리본이 칼끝 **한 점**만 이어 7px 선으로 보였다
#        (사용자 요구: *"리본도 칼날에 맞게 두꺼웠으면 좋겠음"*). 이제 칼끝 궤적과 **칼밑 궤적**
#        사이를 채우는 면(`Polygon2D`)이고, 두 궤적이 같은 배치식에서 나오므로 ③의 성질은 그대로다.
#   ⚠ `swing_fx_*` 배율 3종은 07-29에 삭제했다 — 리본은 칼을 따라갈 뿐이라 조절할 "형태"가 없다.
#     무기별로 갈리는 것은 **`blade_length` 하나**(리본의 radial 폭)이고, 꼬리 모양·알파는
#     `player.gd`의 `TRAIL_*` const가 정본이다(씬의 Gradient/Curve는 2026-08-01에 코드로 옮겼다 —
#     면 리본은 정점 색을 직접 넣어야 해서 Line2D의 gradient/width_curve가 쓸 수 없다).
