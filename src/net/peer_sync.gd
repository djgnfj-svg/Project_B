...
var _peer_stats: Dictionary = {}  # peer_id -> {atk, hp, weapon, lv, ms} — 장비 스탯 + 레벨 5스탯 + 메인 하위 직업 id 공지(G_STATS). 스폰 전 도착 시 버퍼링(_peer_jobs 미러)
...
# 그 피어의 메인 전용 특성(검기 파형) → 평타 사거리 증가율. 미공지/특성 없음 = 0(항등).
# 🔴 공지된 것은 **id뿐**이고 값은 내 data/subjobs에서 리졸브한다(peer_weapon_id 철학 §3).
#   계열은 그 피어가 G_JOB으로 공지한 직업 — 남의 계열 특성을 주장하면 reach_bonus_of가 0으로 폐기한다.
#   G_JOB이 아직 안 왔으면(계열 미상) 0 = 특성 꺼짐 쪽으로 떨어진다. 첫 G_POS 재공지가 곧 복구하고,
#   그때까지는 "정당한 사거리를 잠깐 못 받는" 보수적 방향이라 안전하다(그 반대는 스푸핑 허용).
func peer_reach_bonus(peer_id: int) -> float:
	var st_v: Variant = _peer_stats.get(peer_id)
	if st_v == null:
		return 0.0
	return GameState.reach_bonus_of(
		str((st_v as Dictionary).get("ms", "")), str(_peer_jobs.get(peer_id, "")))
...
func _spawn(peer_id: int, is_local: bool) -> void:
	...
	if _peer_stats.has(peer_id):
		...
		p.set_reach_bonus(peer_reach_bonus(peer_id))  # 메인 특성(파형) — 같은 버퍼에서
	_players[peer_id] = p
...
func _announce_stats() -> void:
	...
	Net.send_game({NetSchema.KEY_KIND: NetSchema.G_STATS,
		...
		# 메인 하위 직업 id — **수치가 아니라 id만** 보낸다(GDD v1.9 특성). 호스트가 자기 data/subjobs에서
		# 리졸브해 사거리 특성을 얻는다(peer_weapon_id 철학 §3 — 사거리 수치를 받으면 그게 스푸핑 표면).
...
func _apply_local_stats() -> void:
	...
	if lp != null:
		...
		# 🔴 내 메인 특성은 _peer_stats가 아니라 GameState에서 온다 — **Net에 루프백이 없어 로컬 항목이
		#   아예 없다.** 성장축 리뷰 Critical(호스트 자기 공속이 게이트에 안 들어가 자기 타격이 걸러졌던 것)과
		#   같은 함정이라, 치명·피흡·공속과 소스를 통일해 둔다.
		lp.set_reach_bonus(GameState.main_reach_bonus())
	_announce_stats()
...
func _on_net_msg(from_id: int, data: Dictionary) -> void:
	match str(data.get(NetSchema.KEY_KIND, "")):
		...
		NetSchema.G_STATS:
			...
			# 메인 하위 직업 id는 **문자열로 보관하고 리졸브는 조회 시점에** 한다(peer_reach_bonus).
			...
			if _players.has(from_id):
				...
				_players[from_id].set_reach_bonus(peer_reach_bonus(from_id))  # 원격 파형 연출·판정 기하
		NetSchema.G_ATK:
			if _players.has(from_id):
