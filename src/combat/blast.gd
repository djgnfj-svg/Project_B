...
# blast.png(64×64)의 원 바깥 반지름(px) — ⚠ 텍스처와 미러. 값은 캔버스 크기가 아니라 **실측**이다:
# 가장자리를 일부러 울퉁불퉁하게(터지는 형태) 그려서 방향별 외곽이 30.0~31.5로 다르다 → 36방향 평균 30.6.
# 캔버스 기준(32)을 쓰면 그려진 원이 판정보다 4% 작아진다("맞는데 안 보임"). 아트를 다시 그리면 재실측할 것.
const TEX_RADIUS := 30.6
