...
# 🔴 확대 배율.
# 🔴 **줌은 1.0이다 — 2026-08-02에 사용자가 "1.0으로 확 빼기"를 골랐다.** 같은 날 1.5 → 1.3으로
#   한 번 내렸고, 그 뒤 다시 "카메라 뒤로"가 와서 세 선택지(1.3 유지 / 1.15 / 1.0)를 놓고 고른 값이다.
# 이 값의 이력은 **뒤집힘의 연속**이라 되돌리기 전에 반드시 읽어라:
#   07-26 아트 16px 전환 보정으로 2.0 도입(`a29991a`) → 07-27 아트 32px 복귀로 1.0 → 08-02 궤적
#   질감·손맛을 화면에서 읽히게 하려고 1.5 → 같은 날 1.3 → 같은 날 **1.0**.
# 🔵 **1.0의 이점은 시야만이 아니다 — 정수배라 픽셀이 균일해진다.** 1.5·1.3은 비정수배라 픽셀 1개가
#   어디선 2px, 어디선 1px로 늘어났다(그 대가를 "넓은 시야가 낫다"는 이유로 감수하고 있었다).
#   1.0에서는 그 대가가 아예 사라진다 — 시야(640×360)와 픽셀 정합을 **둘 다** 얻는다.
# ⚠ **잃는 것**: 08-02에 1.5를 고른 근거였던 「궤적 리본·칼 잔상 질감이 화면에서 읽힌다」가 약해진다.
#   실기에서 궤적이 안 읽히면 되돌릴 자리는 이 상수 하나이지만, **판정 기하는 한 픽셀도 안 움직인다**
#   (줌은 순수 표시축이다). 손맛이 모자라면 줌이 아니라 잔상 밀도·리본 알파를 먼저 봐라.
# ⚠ HUD는 CanvasLayer라 영향을 안 받는다.
# ⚠ 올리면 보이는 범위가 그만큼 줄어 맵이 넓게 느껴지고 같은 move_speed가 더 빠르게 보인다 —
#   아래 두 상한도 배율만큼 나눠야 한다(그 미러가 이 파일 안에 있다 = ZOOM 유도).
const ZOOM := 1.0
...
# 🔴 **그래서 상수로 적지 않고 ZOOM에서 유도한다**(2026-08-02). 전에는 `4.67 = 7.0/1.5`,
#   `6.0 = 9.0/1.5`를 **손으로 계산해 박아** 뒀고 그 나눗셈이 주석에만 있었다 — 줌을 1.5 → 1.3으로
#   내리는 순간 두 값이 조용히 낡아 화면 흔들림이 의도보다 13% 작아진다(에러 없음, 손맛으로만 드러남).
#   분자(7.0 · 9.0)가 "줌 1배에서의 화면 px"이라는 뜻이고, 그것이 이 두 값의 진짜 정본이다.
const SHAKE_MAX := 7.0 / ZOOM   # 오프셋 상한(px) — 640×360 기준, 줌 좌표계로 환산
...
const KICK_MAX := 9.0 / ZOOM # 반동 변위 상한(px) — 줌 좌표계(위 유도 주석과 같은 이유). 셰이크와 합쳐도 화면이 안 무너지는 선
