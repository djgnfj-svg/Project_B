...
@onready var _inv_label: Label = $Inv
...
# 골드 + 재료 총 개수 간단 표시 (inventory_changed 훅). 재료는 전 종류 수량 합.
func _refresh_inv() -> void:
	var mat_total := 0
	for id: String in GameState.materials:
		mat_total += int(GameState.materials[id])
	_inv_label.text = "골드 %d · 재료 %d" % [GameState.gold, mat_total]
