...
- 🔴 **두 번째 콘(부채꼴) 패턴을 추가하기 전 (2026-07-26):** 콘 텔레그래프는 **`half_angle`이 다르면 텍스처도 새로 그려야 한다** — §3 텍스처 계약 4조항(정원 부채꼴·반지름=tex_w·전체각=2×half_angle·테두리 내측)이 균일 스케일에 기대고 있어서, 한 텍스처로 임의 각을 덮는 우회로(세로 비균일 스케일)는 정원을 타원으로 만들어 **다시 어긋난다**(두 번 시도해 두 번 기각). 지금은 콘 패턴이 망령 `swing` 하나뿐이라 "패턴별 정확 텍스처"가 가장 싸고, `BossPatternDef.telegraph_tex`가 이미 패턴별이다. 콘이 둘 이상이 되면 그때 ⑴ 각별 텍스처를 몇 장 두고 데이터가 고르게 할지 ⑵ 셰이더로 각을 uniform으로 뺄지(⚠ 웹 Compatibility + "효과 끝나면 `material=null`" 규율 §5) 결정하고 여기에 기록한다. **`half_angle`은 데이터만 바꿔도 아트 부채가 생기는 유일한 필드다** — 그 사실 자체를 잊지 않는 것이 이 게이트의 목적이다.
...
- 🔴 **보스 패턴 판정 기하 = `combat_math.gd`의 `is_strike_hit()`(원) + `is_hit_in_cone()`(부채꼴).** (보스전 슬라이스 0, 2026-07-23) `BossPatternDef.shape`("circle"/"cone")가 어느 함수를 부를지 정하고, 판정 반경/각(`range`·`half_angle`) = 텔레그래프 표시 스케일/회전 — "맞는 곳=보이는 곳". 물 뿌리기(여러 원 착탄)는 `is_strike_hit`를 착탄점마다 N회(`burst_count`). 새 판정 형태(사각 등)가 필요하면 여기 단일 소스로 추가하고 shape 값을 늘린다.
  - 🔴 **부채꼴 텔레그래프 텍스처 계약 = 조항이 넷이다 (2026-07-26 전면 재작성).** `boss.gd`의 콘 스케일은 **균일**(`range / tex_w`)이라, 판정(`is_hit_in_cone` = 반지름 ∩ 각)과 맞으려면 텍스처가 아래를 **전부** 지켜야 한다. `BossPatternDef.telegraph_tex`는 패턴별이므로 각이 다른 패턴은 텍스처도 따로 그린다.
    1. **형태 = 정원 부채꼴(끝단이 호).** 🔴 **삼각형(평평한 끝단)은 틀리다** — 꼭짓점→모서리 거리가 `range / cos(half_angle)`이 되어 대각선 방향으로 과예고가 생긴다(0.75rad·range 130이면 178px vs 판정 130px = **48px 과예고**).
    2. **반지름 = `tex_w`** (apex에서 호까지 = 텍스처 폭). 균일 스케일 `range / tex_w`가 곧 "반지름 = range"가 되는 근거다.
    3. **전체각 = 2 × `half_angle`** (apex 기준).
    4. **테두리·글로우는 경계 *안쪽*으로 먹인다.** 바깥으로 나가면 각과 반지름이 그 두께만큼 부푼다 — 실제로 4px 테두리가 캔버스 상하에 눌려 각이 **1.98° 초과**한 적이 있다(2026-07-26).
    - **정렬 규약:** apex = 좌측 중앙(`offset = (0, -tex_h*0.5)` + `rotation = angle`이 이걸 전제). `Sprite2D.offset`은 스케일 **전** 로컬이라 apex가 노드 원점에 남고, Godot 2D는 스케일→회전 순서라 회전해도 전단이 없다(2026-07-26 확인).
    - 🔴 **"세로만 비균일 스케일해서 임의 `half_angle`을 한 텍스처로 덮는" 것은 안 된다** — 정원 부채꼴이 **타원**이 되어 축 방향 말고는 다시 어긋난다. 이 우회로를 두 번 시도했으니(리드가 지시했고 art가 실측으로 반박했다) 다시 꺼내지 마라. 각이 다른 패턴 = 텍스처 한 장이 정답이다.
    - ✅ **해소 (2026-07-26):** `telegraph_cone.png`를 **80×110 · 정원 부채꼴 · 반지름 = `tex_w` · 전체각 2×0.75rad**로 재작화 → 망령 swing(`half_angle 0.75`)과 각·반경 정합. 코드 변경 0줄. 이전 상태는 68.6° 텍스처 vs 85.9° 판정으로 **좌우 각 8.6°(range 130에서 19px)가 무예고 피격**이었다.
    - ⚠ **기하 검증은 "어느 알파 계층·어느 x 범위에서 쟀는지"를 반드시 밝혀라.** 알파를 구분하지 않고 최대각을 잡으면 **apex 근처 테두리 픽셀**이 걸려 87.9°(= 2° 초과)로 보인다 — apex에서 19px 거리에선 1픽셀이 각 2.3°에 해당하는 양자화 한계이고, 스케일 후에도 그 지점은 `body_radius` 안이라 플레이어가 경계를 판단하는 곳이 아니다. 의미 있는 값은 **끝단 근처의 채움 경계**(실측 86.37° = 목표 대비 range 130에서 0.49px). 이 구분을 안 하면 정확한 텍스처를 "위반"으로 오판해 불필요한 재작화를 지시하게 된다(2026-07-26에 리드가 실제로 그랬다).
    - **교훈 (두 번 값을 치렀다):** 이 문서가 오래 **각도만** 적어 두고 조항 1·2를 빠뜨려서, 그것을 읽은 사람이 텍스처를 **삼각형으로 오독**했다. 계약을 "각도"로만 요약하면 형태·반지름이 조용히 갈라진다. 원 텔레그래프는 균일 스케일=반경이라 이 함정이 전부 없다.
