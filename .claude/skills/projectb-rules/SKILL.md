...
- **두 번째 메인 전용 특성을 추가하기 전 (v1.9 특성 축 2026-07-26):** 지금은 특성이 하나(검기 파형 = 사거리)라 `SubJobDef.main_reach_bonus` **필드 하나 + 표시 문구가 UI 코드에 하드코딩**돼 있다. 두 번째가 생기면 ⑴ 특성 이름·문구를 `SubJobDef`로 옮기고(UI가 데이터를 읽게) ⑵ **하위 직업당 최대 1개** 규약(GDD §5)을 어기지 않는지 보고 ⑶ 🔒 특성 경계를 다시 읽는다 — **무기·모션·조작을 바꾸는 특성은 거부한다**(그건 2차 전직 = 비범위). 공격력·체력도 금지(장비 축). ⚠ 특성이 곱셈으로 데미지에 걸리면(예: "치명 시 추가타") 화력 예산 밖에 둘 수 없다 — 그때는 GDD §6 예산표부터 고친다(planner 승인).
- 🔴 **G_STATS 공지의 "보유 검증"이 없다 — 특성이 이 표면을 넓혔다** (2026-07-26): `"ms"`(메인 하위 직업 id)는 **발신자 트러스트**다. 전사 계열이면 검성을 **보유하지 않아도** `"ms":"warrior_swordmaster"`를 실어 사거리 +30%를 얻을 수 있다(계열 불일치·모르는 id·상한 초과는 막지만 소유는 안 본다). 현재 수용 이유 = atk/hp 사칭과 **같은 한 장의 공지**이고, 협동 2인·아이템 이전 자유라 남에게 피해가 없으며 상한이 +50%로 묶여 있다. ⚠ 다만 사거리는 **판정 기하**를 바꾸므로 수치 사칭보다 체감이 크다 — 4인/PvP 게이트(아래)에서 `G_STATS` 본격 검증을 할 때 **보유 하위 직업 확인을 같이** 넣는다.
...
- 🔴 **평타 사거리 = `combat_math.gd`의 `effective_attack_range(job, reach)` — 사거리 3함수(`is_hit_in_reach`·`attack_center_offset`·`attack_radius`)가 전부 이 함수를 지난다.** (메인 전용 특성 = 검성 검기 파형, GDD v1.9, 2026-07-26) `reach` = 메인 하위 직업의 `SubJobDef.main_reach_bonus`(0.3 = +30%). **`haste_scale`과 같은 이유로 배율 함수를 공유하는 것 자체가 계약의 증명이다** — 한 곳이라도 `job.attack_range`를 직접 읽으면 판정과 표시(스워시 스케일·파형 도달점)가 갈라져 "맞는 곳 ≠ 보이는 곳"이 되고, 그건 에러 없이 손맛으로만 드러난다. 기본값 0 = 특성 도입 전과 **완전 항등**.
...
- 새 하위 직업 = `data/subjobs/*.tres` 한 장 (`SubJobDef` — 계열(`series_id`)·순서(`order`)·해금 레벨·5스탯 레벨당 스텝). 스프라이트·무기·모션 불필요(GDD §5 — 하위 직업은 성장 갈래다). ⚠ **총 화력 예산이 고정이므로 개수를 늘리면 기존 하위 직업의 레벨당 스텝을 재역산해야 한다** — "재역산이 딸린 한 장"(GDD §6·§7). 재역산 공식 = `GameState.max_level_stats()`(가장 센 하나를 메인 + 나머지 서브 = `w·Σ + (1-w)·max`)가 GDD §6 목표(치명 20%·치명피해 50%·공속 25%·이속 15%·피흡 6%) 이하. **`test_game_state_auto`의 예산 트립와이어가 이걸 지킨다** — 재역산을 잊으면 빨개진다(현 3개 체제 실측: 0.195/0.470/0.225/0.130/0.057).
- ⚠ **메인 전용 특성이 딸린 하위 직업은 "파일 한 장"이 아니다** (GDD v1.9). 특성은 판정·표시·공지 경로를 함께 건드린다 — 검기 파형(사거리)이 그 예다(§3 사거리 계약). 새 특성을 만들 땐 아래 §2 게이트를 먼저 본다.
