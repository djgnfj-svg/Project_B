...
- **모든 손맛/소리는 EventBus 훅에서 파생된다 — 판정 코드에 연출을 섞지 마라.** 훅: `combat_impact(kind, world_pos, amount)`(HP 감소 표시 경로 = `Health.hp_changed(dropped=true)`에서 피격 당사자 글루가 emit — **호스트/게스트 모두** 발화하므로 각 클라가 자기 화면 연출·소리를 재생), `screen_shake(strength)`, `player_swing`·`player_roll`·`entity_died(kind, pos)`. 새 연출은 이 시그널을 **구독**한다.
...
- **스테이지 중 재합류(현재 GDD 비범위) 도입 전:** 합류 피어용 상태 스냅샷(ehp·php 일괄 재송신)을 먼저 만든다 — 없으면 놓친 사망 브로드캐스트가 게스트 화면에 멈춘 좀비 잔몹으로 영구 잔류한다 (2026-07-22 리뷰). ⚠ **드랍도 스냅샷 대상** — 재합류 피어는 기존 바닥 드랍(DropAuthority `_drops` 레지스트리)을 받아야 한다.
- **4인 파티 / PvP성 요소 도입 전:** 픽업 근접 검증을 `DropAuthority._confirm_pickup`에 추가한다 — 요청자 `net_anchor()`와 `_drops[did].pos` 거리를 픽업 반경으로 검증(hit_req의 `is_hit_in_reach` 패턴). 현재는 **공유+선착+2인+아이템 이전 자유**라 생략(게스트가 임의 did로 선점 가능하나 복제·귀속 스푸핑은 없음 — reviewer Important 2026-07-23). DropAuthority에 peer_sync 참조가 필요해진다.
