...
- 🔴 **구르기 타이밍 = `src/core/combat_math.gd`의 `ROLL_TIME_S`·`ROLL_COOLDOWN_S`·`is_roll_grant_ok()`·`is_iframe_active()`.** 로컬 구르기 이동(player)과 호스트 i-frame 검증(G_ROLL 그랜트)이 같은 상수를 읽는다 — player.gd에 사본을 만들면 첫 손맛 튜닝에서 무적 창과 이동이 갈라진다. (i-frame 있음 = 사용자 확정 2026-07-22)
