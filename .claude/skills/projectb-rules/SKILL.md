...
- 🔴 **보스 패턴 판정 기하 = `combat_math.gd`의 `is_strike_hit()`(원) + `is_hit_in_cone()`(부채꼴).** (보스전 슬라이스 0, 2026-07-23) `BossPatternDef.shape`("circle"/"cone")가 어느 함수를 부를지 정하고, 판정 반경/각(`range`·`half_angle`) = 텔레그래프 표시 스케일/회전 — "맞는 곳=보이는 곳". 물 뿌리기(여러 원 착탄)는 `is_strike_hit`를 착탄점마다 N회(`burst_count`). 새 판정 형태(사각 등)가 필요하면 여기 단일 소스로 추가하고 shape 값을 늘린다.
  - 🔴 **부채꼴 텔레그래프 각은 코드가 강제 못 하는 art/data 계약이고, 지금 실제로 어긋나 있다** (2026-07-23 리뷰 등록 → 2026-07-26 실측으로 위반 확인): `boss.gd`의 균일 스케일은 텍스처 **길이=range**만 판정과 맞춘다. **각도는 `telegraph_cone.png`에 그려진 부채꼴 각이 그대로 결정**되므로 텍스처를 **전체각 = 2×half_angle**로 그려야 "보이는 각=맞는 각"이다.
    - 🔴 **실측 위반 (미해소):** `telegraph_cone.png`(80×96)에 그려진 각은 **half 0.599rad(전체 68.6°)** 인데, 현행 보스 망령의 swing은 **0.75rad = 전체 85.9°** 다 → 보이는 예고보다 판정이 **좌우 각 8.6° 넓다**(range 130px에서 약 19px 폭 = 캐릭터 16px보다 넓은 무예고 피격 영역). 텍스처는 옛 보스(전체각 68.8° 설계)에 맞춰 그려졌고, 보스가 교체될 때 함께 갱신되지 않았다. 해소 = ⑴ 망령 `half_angle`을 0.6으로 내리거나(밸런스 변경) ⑵ 86° 콘 텍스처를 새로 그려 `BossPatternDef.telegraph_tex`로 그 패턴에만 지정한다(패턴별 텍스처 필드가 이미 있다).
  ...
  - 🔵 망령 시트엔 **`walk`가 없어**(idle/swing/slam/spray/death) `_has_anim()` 가드가 건너뛰므로 **보스가 이동할 때 idle 자세로 미끄러진다**(에러 없음). 해소 = walk 4프레임 추가(projectb-art).
