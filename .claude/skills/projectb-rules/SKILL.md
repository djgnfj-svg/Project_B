...
- **`server/relay`** (src 밖) — WebSocket 중계 서버(헤드리스 GDScript, 배포 대상). **게임 로직 금지** — 방 코드 스코프의 릴레이만. 스키마는 core `net_schema.gd`를 preload (§3). 실행: `-s res://server/relay/relay_server.gd -- --port=9080`.
  - ⚠ **공개 배포 전 필수(스파이크 미구현):** ① wss(TLS — HTTPS 페이지에서 ws://는 혼합 콘텐츠로 차단됨; 리버스 프록시로 종단), ② 자원 상한(연결 수·클라당 방 수·릴레이 페이로드 크기), ③ 하트비트/좀비 피어 정리.
...
- **웹 실기 자동 검증 = 디버그 브리지**: 브라우저 자동화는 합성 키를 Godot에 못 꽂는다(신뢰 이벤트만 수신, 홀드 불가). `?debug=1`일 때만 `window.pb_press/pb_release(액션)`이 열린다(`src/core/debug_bridge.gd`) — 마우스는 실클릭이 그대로 먹힌다. 로컬 입력 시뮬레이션일 뿐 신뢰 경계(§3)는 그대로.
