...
- **두 번째 무기 모션(찌르기·발사 등 비-스윙) 착수 전:** 공격 **모션 타입**(3박자 호 스윙)이 `player.gd`에 아직 하드코딩돼 있다 — 찌르기/발사 같은 다른 동작을 넣으려면 모션 타입 필드(swing/thrust/shoot)를 `EquipDef`로 빼고 `_update_weapon`을 타입 분기하고 나서 착수한다. (2026-07-24 진척: **궤적 FX 텍스처·스윙 파라미터(호 반각·창·내지르기)·손맛(색·SFX·셰이크)은 이미 `EquipDef`로 데이터화** — swing_texture/swing_arc/swing_time/swing_lunge/swing_color/swing_sfx/hit_sfx/hit_shake. 스윙형 무기 추가는 게이트 없이 .tres 한 장으로 가능). (2026-07-22 등록).
- **스테이지 중 재합류(현재 GDD 비범위) 도입 전:** 합류 피어용 상태 스냅샷(ehp·php 일괄 재송신)을 먼저 만든다 — 없으면 놓친 사망 브로드캐스트가 게스트 화면에 멈춘 좀비 잔몹으로 영구 잔류한다 (2026-07-22 리뷰). ⚠ **드랍·장비 스탯도 스냅샷 대상** — 재합류 피어는 기존 바닥 드랍(DropAuthority `_drops`) + 각 피어의 `G_STATS`(장비 스탯)를 받아야 한다.
