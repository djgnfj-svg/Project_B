...
- **스테이지 중 재합류(현재 GDD 비범위) 도입 전:** 합류 피어용 상태 스냅샷(ehp·php 일괄 재송신)을 먼저 만든다 — 없으면 놓친 사망 브로드캐스트가 게스트 화면에 멈춘 좀비 잔몹으로 영구 잔류한다 (2026-07-22 리뷰). ⚠ **드랍·장비 스탯도 스냅샷 대상** — 재합류 피어는 기존 바닥 드랍(DropAuthority `_drops`) + 각 피어의 `G_STATS`(장비 스탯)를 받아야 한다. ⚠ **비행 중 화살(CombatAuthority `_arrows`)도 검토** — 재합류 피어는 이미 날아가는 화살의 G_SHOOT를 놓쳐 표시가 없다(표시 전용이라 무해하나 일관성, 2026-07-24 리뷰).
...
- (통과: 두 번째 무기 모션 게이트 2026-07-24 — 공격 **모션 타입**을 `EquipDef.motion_type`(swing/shoot/thrust)로 빼고 `player.gd _do_attack`이 타입 분기(`_swing_attack`/`_fire_arrow`). **shoot = 궁수 활 = 실제 이동 투사체**: 표시 화살 = `src/combat/arrow.gd`(순수 연출, 충돌 없음)·전 클라 관리 = `src/stage/arrow_field.gd`·호스트 권한 판정 = `combat_authority.gd`(매 프레임 전진·`is_arrow_hit` 거리 질의·G_ARROW_HIT). 새 발사형 무기 = `motion_type="shoot"` + `arrow_range` .tres 한 장. thrust는 아직 미구현 — 추가 시 `_do_attack`에 분기 하나 더. 네트워크 리뷰 2회 Critical 0.)
...
- 새 발사형(원거리) 무기 = `EquipDef.motion_type="shoot"` + `arrow_range`(사거리) 한 장. 화살 스폰·호스트 판정·G_SHOOT·표시 화살은 **shoot 공용 경로(player `_fire_arrow`·arrow_field·combat_authority)가 자동 처리** (§3 투사체 계약) — 새 활/석궁은 .tres만. ⚠ 스윙형 손맛 필드(swing_*)는 shoot에서 무시됨, `swing_sfx`만 발사음으로 재활용
