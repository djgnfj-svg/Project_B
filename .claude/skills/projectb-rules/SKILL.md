...
- 🔴 **공격 애니 타이밍 미러 = `src/player/player.gd`의 `ATTACK_ANIM_TIME`(0.25) ↔ `*_frames.tres`의 attack 애니 길이(4프레임/speed 16 = 0.25s) < 모든 `data/jobs/*.tres`의 `attack_cooldown`.** 3중 미러다(2026-07-22 리뷰 등록): ① 창 상수와 애니 길이가 어긋나면 애니가 잘리거나 마지막 프레임에 얼어붙고(loop=false), ② 어떤 직업이든 attack_cooldown이 0.25 밑으로 내려가면 원격 애니 창-잠금 가드(`play_attack_fx`)가 정당한 연속 공격의 애니를 무시해 로컬-원격 화면이 갈라진다. attack_cooldown을 조이거나 attack 애니 프레임/speed를 바꿀 땐 셋을 같이 본다. (attack 애니는 전사만 — 없는 직업은 `_has_attack_anim()` 폴백)
