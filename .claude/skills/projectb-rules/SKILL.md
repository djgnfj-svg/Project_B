...
- 원칙: **권한·동기화 로직은 복사 금지 — 두 번째 사용처가 생기기 전에 공용화한다.** 게이트를 통과(분리 완료)하면 해당 줄을 지운다. 새 이음새가 리뷰에서 발견되면 여기 추가한다. (통과: 두 번째 씬 게이트 2026-07-22 — 피어/직업 동기화 = `src/net/peer_sync.gd`, 전투 확정 = `src/stage/combat_authority.gd`, 공용 HUD = `src/hud/stage_hud.tscn`. 마을·스테이지는 이 컴포넌트를 자식 노드로 문다 — 새 씬도 같은 방식, 복사 금지.)
