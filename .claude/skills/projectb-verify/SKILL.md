...
#   fast(유실 허용) 채널 분류 = {G_POS, G_MOB_POS} 정확 일치 + **사건 kind 전량 제외 단정**(하나라도 끼면 빨개진다)
#   ⚠ 사건 kind 목록의 정본은 `tests/test_net_transport_auto.gd`의 `events` 배열이다 — 개수를 이 문서에 적지 마라(새 kind가 늘 때마다 갈라진다)
...
`Net._p2p_available()`이 `OS.has_feature("web")`로 막혀 있어 네이티브 `-s` 경로는 릴레이만 탄다. 즉 **스위트가 전부 그린이어도 P2P에 대해서는 아무것도 검증되지 않았다** — 회귀 0의 증거일 뿐이다(그게 이 게이트의 목적이기도 하다). 아래는 웹 2클라 실기로만 확인된다:
